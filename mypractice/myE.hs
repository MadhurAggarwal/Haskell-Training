...
{-
  Successful Calls include: 
    ...
    unsafeReadInt " 007" = 7
    unsafeReadInt "- 20" = -20
    unsafeReadInt "9223372036854775807" = 9223372036854775807
    unsafeReadInt "9223372036854775808" = -9223372036854775808

    So here, as long as the input string is an Integer (even if outofbounds of Int), has preceeding spaces or 0
    then the unsafeReadInt will get the correct Int as output
    In any other case, it crashes
  
  Unsuccessful Calls include:
    unsafeReadInt "20.2"
    unsafeReadInt "0 20"
    unsafeReadInt "abc"
    unsafeReadInt " 0 - 20"
    ...
    In all these cases we get a Crash with a No Parse Exception:
...
{-
  example:
  readLnMaybe :: IO (Maybe Int)    = 123    -> Just 123
  readLnMaybe :: IO (Maybe Float)  = 12.1   -> Just 12.1
  readLnMaybe :: IO (Maybe String) = "ashe" -> Just "ashe"
-}
...
sumTwo :: IO ()
sumTwo = do 
  one <- askInt "Please enter first number:"
  two <- askInt "Please enter second number:"
  putStrLn ("The sum of both numbers is " ++ show (one + two) ++ ".")
...
replicateM num ioa = 
  if num > 0
    then liftA2 (:) ioa (replicateM (num-1) ioa)
    else return []
...
sumMany = do
  ...
  res <- replicateM n (askInt "Enter next number:")
  putStrLn ("The sum of all numbers is " ++ show (sum res) ++ ".")
...
dice (D q d)      = do
  res <- replicateM q (randomRIO (1,d))
  return (sum res)
...
diceRange (Const num) = (num, num)
diceRange (D q d) = (q, q * d)
diceRange (Plus dA dB) = (minA + minB, maxA + maxB)
  where 
    (minA, maxA) = diceRange dA 
    (minB, maxB) = diceRange dB 
...
shuffle = error "TODO"
-- shuffle list = do 
--   num <- randomRIO (1, length list)
--   let (x, xs) = extract num list 
--   res <- shuffle xs 
--   return (x : res)

extract :: Int -> [a] -> (a, [a])
extract = error "TODO"
-- extract _ []         = error "Extracting element from Empty List"
-- extract num (x : xs) = 
--   if num == 1
--     then (x, xs)
--     else (element, x : list)
--       where 
--         (element, list) = extract (num - 1) xs
