...
{-
  null2 requires the list xs to be of type Eq a, so that it is comparable to []
  so, it has type as null2 :: Eq a => [a] -> Bool
  this additional restriction makes it worse than null, which doesnt impose any additional constraints

  null3 requires the entire list to be traversed. This is very inefficient
  it cannot compute answer for infinite lists like [1..]
-}
